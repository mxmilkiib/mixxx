...
- 2. **Promotion check** — At the start of any session, verify promotion is working:
  - **Promotion currency**: `integrated` MUST NOT lag behind a passing `integrating`, and MUST NOT run ahead of it either. Promotion is automated via `.github/workflows/mixxx-milkii-integration-auto-promote.yml` — when `develop.yml` CI completes on `integrating`, it checks job results and promotes `integrating` → `integrated` via `gh api` ref update if the run passed cleanly OR all failures match `KNOWN_INFRA_FAILURES` (same patterns as the script).
  - **Invariant**: `integrated` is a CI-confirmed snapshot of `integrating` — it MUST always be an ancestor-or-equal of `origin/integrating`, never ahead. `integrating` advances forward; `integrated` is the point `integrating` had reached when CI last passed. Verify with `git rev-list --left-right --count origin/integrating...origin/integrated`; a non-zero right-hand count means a commit reached `integrated` without passing the CI gate.
    ...
    - **Auto-promote survival**: `mixxx-milkii-integration-auto-promote.yml` MUST exist on `integrating` as well as on `integrated`. Auto-promote force-updates `integrated` to the `integrating` SHA; if the workflow file is only on `integrated`, the first successful promotion removes it and the trigger never fires again.
    - Check with `gh run list --workflow mixxx-milkii-integration-auto-promote.yml --repo mxmilkiib/mixxx --limit 1 --json status,conclusion`; empty output means "never ran", which is NOT the same as "working".
...
- 7. **Rebase + remerge integration + build + test + push** — all via `--rebase-merge-test-push`
  ...
  - automated via `./mixxx-milkii-integration-update-branches.sh --rebase-merge-test-push` (run from `~/src/mixxx-dev/integration/`)
    - the script: (1) rebases all worktrees, (2) resets `integration` to `upstream/main` and merges all `INTEGRATION_BRANCHES` in order, (3) builds test binaries, (4) runs tests, (5) pushes `integration` then `integrating`.
    - a merge conflict in step 2 with genuinely unresolved paths is fatal — the script exits non-zero with a CONFLICT message. If git rerere has staged a resolution and no unmerged paths remain, the merge is committed automatically. Resolve unresolved conflicts manually in the integration worktree, then re-run.
...
- 9. **Integration remerge** — automated by `--rebase-merge-test-push` (and standalone `--remerge-integration`)
  ...
  - **Merge conflicts**: if a merge conflicts and git rerere has staged a resolution with no unmerged paths remaining, the script commits the rerere-resolved merge automatically and continues. Only conflicts with genuinely unresolved paths abort the remerge with a CONFLICT message. Resolve manually in the integration worktree (`cd ~/src/mixxx-dev/integration && git merge --no-edit <branch>`, resolve, `git add`, `git commit`). **git rerere** records the resolution — subsequent remerges auto-resolve the same conflict. Then re-run `--rebase-merge-test-push` or continue with `--build-all-tests && --run-tests && --push-integrating`.
  - **Rerere scoping**: rerere is NOT globally enabled. All worktrees share one `.git/config`, so global rerere could fire during a feature branch `git rebase upstream/main` and auto-apply an integration-merge resolution that combines content from multiple branches — polluting the feature branch with unrelated changes that could reach an upstream PR. The script enables rerere only inside `remerge_integration()` and disables it after. `rebase_all()` explicitly disables it before rebasing any feature branch. The only window where rerere is enabled outside `remerge_integration` is after a conflict abort (so the user can record a manual resolution) — `rebase_all()` closes that window on the next run.
  - **Infrastructure preservation**: `git reset --hard upstream/main` at the start of remerge affects two categories of integration-only files. (a) Files that don't exist in upstream/main (`mixxx-milkii-integration-auto-promote.yml`, `mixxx-milkii-integration-manjaro-release.yml`, `INTEGRATION.md`, the three helper scripts) — gone after reset, restored from the pre-reset SHA. (b) Files that exist in upstream/main but have integration-specific modifications (`develop.yml`, `pre-commit.yml` — branch exclusions, concurrency settings) — reset replaces them with upstream versions, so the script compares blob SHAs before and after reset and restores any file whose content drifted. Without this, the promotion chain breaks on the next push because `mixxx-milkii-integration-auto-promote.yml` vanishes and `develop.yml` loses its integration-specific branch exclusions.
  ...
  - **Force-push semantics**: `push_integration` and `push_integrating` both use `--force-with-lease` because the remerge rewrites `integration` history from scratch (reset + re-merge produces new merge commit SHAs). `--force-with-lease` rejects the push if the remote moved unexpectedly (e.g. someone else pushed), preventing accidental overwrites.
...
- 10. **Test binary completeness**: Before `--push-integrating` can succeed, ALL non-skipped worktree branches MUST have a `build/mixxx-test` binary AND each must have a per-branch sentinel with an acceptable status for its current HEAD. `--rebase-merge-test-push` uses `--build-all-tests` which: (1) launches all cmake configures **in parallel** (configure is I/O-bound; 16 configures take ~5s instead of ~70s serial), then (2) builds `mixxx-test` serially with `-j(nproc-2)`, then (3) prints a `ccache -s` summary. Standalone: `--build-all-tests` then `--run-tests`. The script lists all blocking branches when `--push-integrating` is blocked. `--rebuild-tests` still exists but only handles stale binaries (ldd-detected) — it does NOT configure unconfigured branches.
  ...
  - `ControllerScriptEngineLegacyTest.*:ControllerScriptEngineLegacyTimerTest.*:AdjustReplayGainTest.AdjustReplayGainUpdatesPregain:TrackMetadataExportTest.keepWithespaceKey:MidiMappings/.*:HidMappings/.*:BulkMappings/.*` (colon-separated, matching `KNOWN_FAILING` in the script and pre-push hook). These suites are excluded for the same state-poisoning / known-failing reasons documented in the **Upstream test filter scope** and **Pre-push hook timeout** rules. A test failure fails the workflow before any package is produced.
  - Wait for GA CI on `origin/integrating`. The `mixxx-milkii-integration-auto-promote.yml` workflow fires automatically when CI completes and promotes `integrating` → `integrated` if all failures are known infra/flaky.
  ...
  - If any failure is unknown, auto-promote blocks; investigate whether it is pre-existing on `upstream/main` or integration-introduced, and add the job pattern to `KNOWN_INFRA_FAILURES` (in both the script and `mixxx-milkii-integration-auto-promote.yml`) only after confirming it is infra/flaky.
...
- **Manjaro release pipeline**: `.github/workflows/mixxx-milkii-integration-manjaro-release.yml` is a release-packaging workflow distinct from the `build.yml` CI matrix. it produces an installable Manjaro/Arch package from the CI-confirmed `integrated` branch and publishes it as a pre-release GitHub Release on `mxmilkiib/mixxx`.
...
- **Operational notes**
  ...
  - The workflow does NOT block `integrated` promotion; it runs asynchronously after the push. Check status with `gh run list --workflow mixxx-milkii-integration-manjaro-release.yml --repo mxmilkiib/mixxx --limit 3`.
...
- **CI status** — fresh CI run triggered on `origin/integrating` (af0527fbc5), pushed 2026-08-20 after
  ...
  `mixxx-milkii-integration-auto-promote.yml` is present on both `integrating` and `integrated`; auto-promote should fire when CI
  ...
  Script fixes applied 2026-08-20 (permanent, documented in relevant sections above): remerge infrastructure
  preservation, rerere-resolved conflict committing, `KNOWN_FAILING` filter expanded to include
  `MidiMappings/HidMappings/BulkMappings` (see **Upstream test filter scope** and **Merge conflicts**).
