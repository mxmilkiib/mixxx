...
- 9. **Integration remerge** — automated by `--rebase-merge-test-push` (and standalone `--remerge-integration`)
  ...
  - **Infrastructure preservation**: `git reset --hard upstream/main` at the start of remerge would destroy files that exist only on the integration branch (`.github/workflows/auto-promote.yml`, `develop.yml`, `manjaro-release.yml`, `pre-commit.yml`, `INTEGRATION.md`, and the three `mixxx-milkii-integration-*.sh` helper scripts). The script records the pre-reset HEAD SHA and restores any missing infrastructure files from it after all merges complete, committing them as "restore integration infrastructure after remerge". Without this, the promotion chain breaks on the next push because `auto-promote.yml` vanishes.
