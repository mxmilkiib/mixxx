...
promote_integrated() {
    ...
    # `git branch -f` refuses a ref checked out in a worktree; reset that
    # worktree instead so its checked-out files track the promoted SHA.
    local integrated_wt
    integrated_wt=$(env GIT_PAGER=cat git -C "$MIXXX_MAIN" worktree list --porcelain |
        awk '$1 == "worktree" { wt = $2 } $1 == "branch" && $2 == "refs/heads/integrated" { print wt }')
    if [[ -n "$integrated_wt" ]]; then
        if ! env GIT_PAGER=cat git -C "$integrated_wt" diff --quiet ||
           ! env GIT_PAGER=cat git -C "$integrated_wt" diff --cached --quiet; then
            status "ABORT promote_integrated: $integrated_wt has tracked changes"
            return 1
        fi
        env GIT_PAGER=cat git -C "$integrated_wt" reset --hard "$expected_sha" || {
            status "ABORT promote_integrated: integrated worktree reset failed"
            return 1
        }
    else
