...
push_changed_branches() {
    ...
    local branch_name name base _rebase _test _integrate _gate _dependency pr_head dir local_pids origin_pids push_target
    while IFS=$'\t' read -r branch_name name base _rebase _test _integrate _gate _dependency pr_head; do
        dir="$MIXXX_DEV/$name"
...
promote_integrated() {
    ...
    while true; do
        ...
        if ! run_line=$(gh run list --workflow "$CI_WORKFLOW" --branch integrating \
                ...
                --jq '.[0] | [.databaseId, .status, (.conclusion // ""), .headSha] | @tsv' 2>/dev/null); then
    ...
    if ! env GIT_PAGER=cat git -C "$MIXXX_MAIN" merge-base --is-ancestor \
        ...
        status "BLOCKED promote_integrated: integrated is not an ancestor of tested integrating"
        return 1
    fi

    env GIT_PAGER=cat git -C "$MIXXX_MAIN" branch -f integrated "$expected_sha"
    if ! env GIT_PAGER=cat git -C "$MIXXX_MAIN" push origin \
            "$expected_sha:refs/heads/integrated"; then
        status "ABORT promote_integrated: fast-forward push failed"
        return 1
    fi
    status "DONE integrated fast-forwarded to CI-tested SHA $expected_sha"
}
...
case "${1:-}" in
    ...
    "")                          rebase_all ;;
    *) echo "Usage: $0 [--validate-manifest | --rebuild-tests | --build-all-tests | --run-tests | --remerge-integration [--dry-run] | --push-changed | --push-integrating | --promote-integrated | --sync-gist | --rebase-merge-test-push | --rebase-merge-test-push-promote]"; exit 1 ;;
esac
